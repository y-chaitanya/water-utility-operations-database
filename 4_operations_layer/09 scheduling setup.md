...
## ⚠️ Honest notes (say these plainly if asked)
- **This is the Express-appropriate method, not SQL Agent.** Express lacks SQL Agent, so I scheduled the procedure with container cron instead. The skill demonstrated — automating a recurring maintenance task — is the same; the standard production tool would be SQL Agent.
- **Container restarts:** if you stop/restart the container, you may need to re-run `service cron start` (the base image doesn't auto-start cron). That's a sandbox quirk, not a flaw in the approach — worth mentioning honestly rather than hiding.
- **Credentials:** the SA password sits in the shell script for the demo. In production this would use a managed identity or a secured credential store, never a plaintext password in a script. Note that openly.
- **Synthetic data:** as with the whole project, everything here runs against synthetic sandbox data.

---

## What this demonstrates (for the interview)
- I understand **what** routine database maintenance is (index upkeep, statistics, capacity tracking) and bundled it into a callable, logged procedure.
- I understand **how** to automate a recurring task — and I know the standard tool (SQL Agent) plus a working alternative when it isn't available (Express + cron).
- I built in an **audit trail** (`Maintenance_Log`) so the automation is verifiable, not assumed — the same "prove it ran" discipline as the rest of the project.
